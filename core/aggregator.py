...
class Aggregator(object):
    ...
    def client_completion_handler(self, results):
        ...
        if self.model_in_update == self.tasks_round:
            for idx, param in enumerate(self.model.state_dict().values()):
                logging.info(f"{param.shape} / {results['update_weight'][idx].shape}")
                logging.info(f"{type(param)} / {type(results['update_weight'][idx])}")
                logging.info(f"{param} / {results['update_weight'][idx]}")
                #if len(param.shape) > 0:
                _ = results['update_weight'][idx]
                logging.info(f"{_}")
                logging.info(f"{type(_)}")
                _ = _ /float(self.tasks_round)
                logging.info(f"{_}")
                logging.info(f"{type(_)}")
                _ =  torch.as_tensor(_)
                logging.info(f"{_}")
                logging.info(f"{type(_)}")
                _ = _.to(device=device)
                logging.info(f"{_}")
                logging.info(f"{type(_)}")
                assert(tuple(param.data.shape) == tuple(_.shape))
                param.data = _.to(dtype=param.data.dtype)
                logging.info(f"{_}")
                logging.info(f"{type(_)}")
                #else:
