...
class AffineGridGenV2(Module):
    def __init__(self, height, width, lr=1, aux_loss=False):
        ...
        self.grid[:, :, 0] = np.expand_dims(np.repeat(np.expand_dims(
            np.arange(-1, 1, 2.0/self.height), 0), repeats=self.width, axis=0).T, 0)
        self.grid[:, :, 1] = np.expand_dims(np.repeat(np.expand_dims(
            np.arange(-1, 1, 2.0/self.width), 0), repeats=self.height, axis=0), 0)
        self.grid[:, :, 2] = np.ones([self.height, width])
        self.grid = torch.from_numpy(self.grid.astype(np.float32))

    def forward(self, input1):
        self.batchgrid = torch.zeros(torch.Size(
            [input1.size(0)]) + self.grid.size())
...
class CylinderGridGenV2(Module):
    ...
    def forward(self, input):
        ...
        output1 = torch.atan(torch.tan(
            np.pi/2.0*(self.batchgrid[:, :, :, 1:2] + self.batchgrid[:, :, :, 2:] * input_u[:, :, :, :]))) / (np.pi/2)
...
class DenseAffine3DGridGen(Module):
    ...
    def forward(self, input1):
        self.batchgrid3d = torch.zeros(torch.Size(
            [input1.size(0)]) + self.grid3d.size())
...
class DenseAffine3DGridGen_rotate(Module):
    ...
    def forward(self, input1, input2):
        ...
        input_u = input2.view(-1, 1, 1, 1).repeat(1,
                                                  self.height, self.width, 1)

        output = torch.cat([theta, phi], 3)

        output1 = torch.atan(torch.tan(
            np.pi/2.0*(output[:, :, :, 1:2] + self.batchgrid[:, :, :, 2:] * input_u[:, :, :, :]))) / (np.pi/2)
...
class Depth3DGridGen(Module):
    ...
    def forward(self, depth, trans0, trans1, rotate):
        ...
        x = self.batchgrid3d[:, :, :, 0:1] * depth + \
            trans0.view(-1, 1, 1, 1).repeat(1, self.height, self.width, 1)

        y = self.batchgrid3d[:, :, :, 1:2] * depth + \
            trans1.view(-1, 1, 1, 1).repeat(1, self.height, self.width, 1)
        ...
        input_u = rotate.view(-1, 1, 1, 1).repeat(1,
                                                  self.height, self.width, 1)
...
class Depth3DGridGen_with_mask(Module):
    ...
    def forward(self, depth, trans0, trans1, rotate):
        ...
        x_ = self.batchgrid3d[:, :, :, 0:1] * depth + \
            trans0.view(-1, 1, 1, 1).repeat(1, self.height, self.width, 1)

        y_ = self.batchgrid3d[:, :, :, 1:2] * depth + \
            trans1.view(-1, 1, 1, 1).repeat(1, self.height, self.width, 1)
        z = self.batchgrid3d[:, :, :, 2:3] * depth
        #print(x.size(), y.size(), z.size())

        rotate_z = rotate.view(-1, 1, 1, 1).repeat(1,
                                                   self.height, self.width, 1) * np.pi
