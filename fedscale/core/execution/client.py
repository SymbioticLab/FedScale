...
class Client(object):
    ...
    def train(self, client_data, model, conf):
                ...
                for data_pair in client_data:
                    ...
                    if conf.task == 'nlp':
                        ...
                        data, target = mask_tokens(
                            data, tokenizer, conf, device=device)
                    ...
                    if conf.task == "detection":
                        ...
                        data, masks = Variable(data).to(
                            device=device), Variable(masks).to(device=device)
                    ...
                    loss_squre = sum([l**2 for l in loss_list]
                                     )/float(len(loss_list))
                    ...
                    self.optimizer.update_client_weight(
                        conf, model, global_model if global_model is not None else None)
        ...
        results['utility'] = math.sqrt(
            loss_squre)*float(trained_unique_samples)
